...
- **Flexible data sources** – Toggle between simulated ticks, real-time quotes, or historical week replays via a single backend `MODE` flag; the UI reads the active mode from the snapshot payload.
...
2. **Configure frontend environment**
   ...
   ```
   Edit `.env.local` to add `VITE_OPENROUTER_API_KEY`. Optionally point `VITE_API_BASE` at a remote backend or override the default ticker set with `VITE_ARENA_TICKERS` (comma-separated symbols).
...
4. **Create backend configuration**
   - Duplicate the root `.env` file or create `backend/.env` with values for `OPENROUTER_API_KEY`, the simulation `MODE`, rate intervals, optional ticker overrides (`ARENA_TICKERS`), and persistence paths (see [Backend Setup](./BACKEND_SETUP.md)).
...
- **Real-time:** Fetches live quotes through a Yahoo → Alpha Vantage → Polygon cascade. Enable by setting `MODE=realtime` in the backend `.env`. Optionally set `USE_DELAYED_DATA=true` to lean on delayed quotes for reduced throttling.
- **Historical week:** Replays a specific trading week end-to-end. Set `MODE=historical` and provide `HISTORICAL_SIMULATION_START_DATE` on the backend.

The frontend inspects the simulation snapshot returned by `/api/simulation/state`, so changing `MODE` server-side automatically updates the UI—no additional Vite flags are required. To adjust the instrument universe, use `ARENA_TICKERS` (backend) and/or `VITE_ARENA_TICKERS` (frontend) with a comma-separated list of symbols.
